...
This file implements a LangChain MCP client that:
  - Loads configuration from a JSON file specified by the LANGCHAIN_MULTI_SERVER_CONFIG environment variable.
...
Detailed explanations:
  ...
  - Environment Variable: LANGCHAIN_MULTI_SERVER_CONFIG should point to a config JSON that defines all MCP servers.
"""
...
def read_config_json():
    ...
    Priority:
      1. Try to read the path from the LANGCHAIN_MULTI_SERVER_CONFIG environment variable.
      2. If not set, fallback to a default file 'langchain_multi_server_config.json' in the same directory.

    Returns:
        dict: Parsed JSON content with MCP server definitions.
    """
    # Attempt to get the config file path from the environment variable
    config_path = os.getenv("LANGCHAIN_MULTI_SERVER_CONFIG")

    if not config_path:
        # If environment variable is not set, use a default config file in the same directory as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(script_dir, "langchain_multi_server_config.json")
        print(f"⚠️  LANGCHAIN_MULTI_SERVER_CONFIG not set. Falling back to: {config_path}")
    ...
    async with AsyncExitStack() as stack:
        ...
        for server_name, server_info in mcp_servers.items():
            ...
            if server_info.get("disabled", False):
                print(f"\nSkipping disabled MCP Server, is disabled: {server_name}")
                continue    
